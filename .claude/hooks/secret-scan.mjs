...
const text = [ti.content, ti.new_string, ti.command]
  // MultiEdit carries its payloads in edits[].new_string — include them or a secret
  // written via MultiEdit bypasses the scan (the matcher is wired for MultiEdit).
  .concat(Array.isArray(ti.edits) ? ti.edits.map((e) => e?.new_string) : [])
